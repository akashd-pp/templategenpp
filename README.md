...
2. Push everything in this folder to the repo root (`index.html`,
   `manager.html`, `timecalc.html`, `style.css`, `manager.css`, `app.js`,
   `templates.js`, `engine.js`, `manager.js`, `customTemplates.js`, and the
   `assets/` folder).
...
A small standalone tool (`timecalc.html`) for working out an end time from
a start time and a duration — e.g. a call started at 10:00 AM and lasted
45 minutes, so it ended at 10:45 AM. Enter a start time and a duration in
hours/minutes; the end time is calculated live in 12-hour AM/PM format,
with a note if the duration crosses into the next day. "Copy result"
copies the start–end range as plain text (e.g. "10:00 AM – 10:45 AM") for
pasting into a ticket or note. Reachable from the "+ Time Calculator" link
in the footer, or the logo/header (which always links back to the main
composer from any page).
